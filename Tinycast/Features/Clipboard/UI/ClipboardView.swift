                    ...
                    ForEach(rows) { row in
                        ...
                        case .item(let item, let shortcutNumber):
                            ClipboardRow(
                                ...
                                allowsCommandNumber: allowsCommandNumbers
                            )
...
private struct ClipboardRow: View {
    ...
    let allowsCommandNumber: Bool
    /// Observed here so a ⌘ press re-renders rows rather than the whole palette.
    ...
    var body: some View {
        HStack(spacing: Theme.Spacing.lg) {
            ...
            Text(previewText)
                ...
                .truncationMode(.tail)
            Spacer(minLength: 0)
            if let shortcutNumber, allowsCommandNumber, palette.commandHeld {
        ...
        .armedHover($hovered)
        .animation(.easeOut(duration: Theme.Duration.tooltip), value: palette.commandHeld)
    }
